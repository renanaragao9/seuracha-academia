...
function openFee(id: number): void {
  navigateTo(`/students/monthly-fees/show?id=${id}`);
}
</script>
...
<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8 relative">
    ...
    >
      ...
      <rect width="100%" height="100%" fill="url(#wave-fees)" />
    </svg>
    ...
    >
      <button
        ...
        @click="openFee(fee.id)"
      >
        <div class="flex items-center gap-3 mb-4">
          <div
            class="h-12 w-12 rounded-xl bg-emerald-500/15 flex items-center justify-center shrink-0"
          >
            <Icon name="lucide:credit-card" class="h-6 w-6 text-emerald-500" />
          </div>
          <div class="min-w-0 flex-1">
            <p class="text-white font-bold truncate">{{ fee.planType.name }}</p>
            <p class="text-sm text-zinc-400 mt-0.5">
              De {{ formatDate(fee.startDate) }}
              <br />Até {{ formatDate(fee.endDate) }}
            </p>
          </div>
          <Icon
