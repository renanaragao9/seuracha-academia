...
namespace App\Filament\Resources\TrainingSheets\RelationManagers;
                ...
                Select::make('training_sheet_division_id')
                    ...
                    ->relationship(
                        ...
                        modifyQueryUsing: fn (Builder $query): Builder => $query
                            ...
                            ->where('training_sheet_id', $this->getOwnerRecord()->getKey())
                            ->orderBy('order')
                    )
                    ->getOptionLabelFromRecordUsing(fn ($record): string => 'Treino '.($record->trainingDivision?->name ?? '-'))
                    ->searchable()
                    ->preload()
                    ->required(),
